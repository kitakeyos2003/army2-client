...
public class Image {
    ...
    public static Image createImage(final String url) {
        ...
        Gdx.app.postRunnable(new Runnable() {
            @Override
            public void run() {
                Texture t = new Texture(Gdx.files.internal(String.valueOf(LibSysTem.res) + url));
                img.texture = t;
    ...
    public static void createImage(final String url, final IAction2 action2) {
        Gdx.app.postRunnable(new Runnable() {
            ...
            public void run() {
                ...
                if (CCanvas.isPc()) {
                    file = Gdx.files.local(String.valueOf(LibSysTem.res) + url);
                } else {
                    file = Gdx.files.internal(String.valueOf(LibSysTem.res) + url);
                }
                t = new Texture(file);
                img.texture = t;
                img.width = img.texture.getWidth();
    ...
    public static Image createImage(final byte[] encodedData, final int offset, final int len, final String path,
            final String name) {
        final Image img = new Image();
        Gdx.app.postRunnable(new Runnable() {
            @Override
