...
public class PlayerEquip {
    ...
    public byte type;
    public byte id;
    public static Vector<EquipGlass> playerData;
    ...
    public PlayerEquip(short[][] equip) {
        (this.equips = new Equip[5])[0] = createEquip((byte) equip[0][0], (byte) equip[0][1], equip[0][2]);
    ...
    public static void addGlassEquip(Vector<EquipGlass> glass) {
        PlayerEquip.playerData = glass;
    }

    public static EquipGlass getEquipGlass(byte id) {
        for (int i = 0; i < PlayerEquip.playerData.size(); ++i) {
            EquipGlass eq = PlayerEquip.playerData.elementAt(i);
    ...
    static {
        PlayerEquip.imgData = new mImage[10];
        PlayerEquip.data = new byte[10][];
        PlayerEquip.header = new byte[10][10][];
        PlayerEquip.bullets = new mImage[10];
        PlayerEquip.headers = new Vector<>();
    }
}
