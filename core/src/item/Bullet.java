...
public class Bullet {
    ...
    int xD;
    int yD;
    boolean isActiveMissile;
    boolean isActiveHammer;
    boolean isActiveEgg;
    boolean isActiveRain;
    byte whoShot;
    private int color;
    boolean rotate;
    boolean translate;
    ...
    boolean changeFrame;
    boolean lazerBullet;
    boolean changeFrameLeft;
    boolean isLightBlink;
    boolean isWaterBum;
    boolean lazerShoot;
    boolean lazerStop;
    boolean tonardo;
    boolean beginTimeCount;
    boolean chickenBull;
    boolean chickenHair;
    ...
    public Vector<Position> postion;
    public mImage dan;
    ...
    public Bullet(byte type, short[] X, short[] Y, byte whoShot) {
        ...
        this.isActiveEgg = false;
        this.isActiveRain = false;
        this.rotate = false;
        ...
        this.smokeDelay = 0;
        this.smoke = false;
        this.longSmoke = false;
        this.fireSmoke = false;
        this.blackSmoke = false;
        this.changeFrame = false;
        this.lazerBullet = false;
        this.changeFrameLeft = false;
        this.isLightBlink = false;
        this.lazerShoot = false;
        this.lazerStop = true;
        this.tonardo = false;
        this.beginTimeCount = false;
        this.chickenBull = false;
        this.chickenHair = false;
        this.sFrame = 0;
        this.ts = 0;
        this.paintCount = 0;
        this.dan = null;
        ...
        this.pw = 1;
        this.ph = 1;
        this.changePositon = false;
        this.dis = 0;
        this.count = 0;
        this.dis_ChPosition = 4;
        this.dis_delay = 0;
        this.timecount = 0;
        this.isfall = false;
        this.dym = 0;
        this.beginUdgr = false;
        this.missileTime = 0;
        this.lastAngle = 0;
    ...
    public void paintLazerGirl(mGraphics g) {
        ...
        if (!this.isMirror) {
            this.xLaser1 += Bullet.dXLaser;
            this.yLaser1 += Bullet.dYLaser;
            ...
            if (mSystem.currentTimeMillis() - this.timeDelayPaint > 20L) {
                this.xLaser2 += Bullet.dXLaser;
                this.yLaser2 += Bullet.dYLaser;
    ...
    public boolean collisionTornado() {
        Vector<Tornado> tornados = BM.vTornado;
        if (tornados.size() >= 0) {
    ...
    public void fixedUpdate() {
        ...
        if (this.type == 49) {
            if (Bullet.dXLaser != 0 && Bullet.dYLaser != 0) {
        ...
        if (this.fireSmoke) {
            int deltaX = CRes.random(-7, 7);
            int deltaY = CRes.random(-7, 7);
            GameScr.sm.addSmoke(this.x + deltaX, this.y + deltaY, (byte) 21);
        }
        ...
        if (this.type == 48 && this.pingFrame < this.postion.size()) {
            Position p = this.postion.elementAt(this.pingFrame);
            int xHit = p.x;
            int yHit = p.y;
        ...
        if (this.fireSmoke) {
            ...
            if (Math.abs(this.x - x1) >= 10 || Math.abs(this.y - y1) >= 10) {
                GameScr.sm.addSmoke((this.x + x1) / 2 + deltaX, (this.y + y1) / 2 + deltaY, (byte) 21);
            }
        }
        ...
        if (this.rotate) {
            ...
            int angle = CRes.angle(this.x - x1, this.y - y1);
    ...
    public void explode(int index) {
        ...
        if (this.type == 34) {
            ...
            CMap newCMap = new CMap(Bullet.webId++, this.x - 21, this.y - 20, CMap.MANGNHEN, true);
            ...
            if (this.type == 47) {
                ...
                for (int j = 0; j < 2; ++j) {
                    ...
                    GameScr.sm.addRock(this.x, this.y, Math.abs(CRes.random(4 + this.force / 3)), -CRes.random(8 + this.force / 3, 10 + this.force / 3), rockType);
                    GameScr.sm.addRock(this.x, this.y, -Math.abs(CRes.random(4 + this.force / 3)), -CRes.random(8 + this.force / 3, 10 + this.force / 3), rockType);
                }
                if (this.type == 3) {
                    new Explosion(this.x - 30, this.y + 5, (byte) 0);
                ...
                if (BM.isActiveTornado) {
                    Vector<Tornado> tonardos = BM.vTornado;
                    for (int k = 0; k < tonardos.size(); ++k) {
                        Tornado t = (Tornado) tonardos.elementAt(k);
                        if (this.x < t.x + 10) {
                            int x = t.x;
                        }
                    }
    ...
    public void paint(mGraphics g) {
        ...
        if (this.isSuper) {
            ...
            g.drawRegion(Bullet.superShoot, 0, 32 * this.sFrame, 32, 32, 0, this.x, this.y, 3, false);
        }
        if (this.dan != null) {
    ...
    static {
        try {
            Bullet.filePack = new FilePack(CCanvas.getClassPathConfig(String.valueOf(CONFIG.PATH_ITEM) + "item"));
            Bullet.bomb = Bullet.filePack.loadImage("bomb.png", new IAction2() {
                @Override
                public void perform(Object object) {
                    CRes.onSaveToFile((Image) object, "bomb");
                }
            });
            Bullet.bomb_flag = Bullet.filePack.loadImage("specialbullet.png", new IAction2() {
                @Override
                public void perform(Object object) {
                    CRes.onSaveToFile((Image) object, "specialbullet");
                }
            });
            Bullet.filePack.loadImage("grenade.png", new IAction2() {
                @Override
                public void perform(Object object) {
            ...
            Bullet.webImg = Bullet.filePack.loadImage("web.png");
            Bullet.filePack.loadImage("chuoi2.png", new IAction2() {
                @Override
                public void perform(Object object) {
            ...
            Bullet.chicken2 = Bullet.filePack.loadImage("gaBull2.png", new IAction2() {
                @Override
                public void perform(Object object) {
                    CRes.onSaveToFile((Image) object, "gaBull2");
                }
            });
            Bullet.rocket2 = Bullet.filePack.loadImage("rocket2.png", new IAction2() {
                @Override
                public void perform(Object object) {
                    CRes.onSaveToFile((Image) object, "rocket2");
                }
            });
            Bullet.egg = Bullet.filePack.loadImage("trungvang.png", new IAction2() {
                @Override
                public void perform(Object object) {
                    CRes.onSaveToFile((Image) object, "trungvang");
                }
            });
            Bullet.egg2 = Bullet.filePack.loadImage("trung2.png", new IAction2() {
                @Override
                public void perform(Object object) {
                    CRes.onSaveToFile((Image) object, "trung2");
                }
            });
            Bullet.mouse = Bullet.filePack.loadImage("chuotBull.png", new IAction2() {
                @Override
                public void perform(Object object) {
                    CRes.onSaveToFile((Image) object, "chuotBull");
                }
            });
            Bullet.meteor = Bullet.filePack.loadImage("thienthach.png", new IAction2() {
                @Override
                public void perform(Object object) {
                    CRes.onSaveToFile((Image) object, "thienthach");
                }
            });
            Bullet.mortar = Bullet.filePack.loadImage("dan.png", new IAction2() {
                @Override
                public void perform(Object object) {
                    CRes.onSaveToFile((Image) object, "dan");
                }
            });
            Bullet.rainmissile = Bullet.filePack.loadImage("13Missile.png", new IAction2() {
                @Override
                public void perform(Object object) {
                    CRes.onSaveToFile((Image) object, "13Missile");
                }
            });
            Bullet.khoang = Bullet.filePack.loadImage("khoang.png", new IAction2() {
                @Override
                public void perform(Object object) {
                    CRes.onSaveToFile((Image) object, "khoang");
                }
            });
            Bullet.daodat = Bullet.filePack.loadImage("daodat.png", new IAction2() {
                @Override
                public void perform(Object object) {
                    CRes.onSaveToFile((Image) object, "daodat");
                }
            });
            Bullet.daodat2 = Bullet.filePack.loadImage("daodat2.png", new IAction2() {
                @Override
                public void perform(Object object) {
                    CRes.onSaveToFile((Image) object, "daodat2");
                }
            });
            Bullet.saobang = Bullet.filePack.loadImage("saobang.png", new IAction2() {
                @Override
                public void perform(Object object) {
                    CRes.onSaveToFile((Image) object, "saobang");
                }
            });
            Bullet.imgGun = Bullet.filePack.loadImage("gunIcon.png", new IAction2() {
                @Override
                public void perform(Object object) {
                    CRes.onSaveToFile((Image) object, "gunIcon");
                }
            });
            Bullet.nuke = Bullet.filePack.loadImage("bigMissile.png", new IAction2() {
                @Override
                public void perform(Object object) {
                    CRes.onSaveToFile((Image) object, "bigMissile");
                }
            });
            Bullet.lazer = Bullet.filePack.loadImage("laser.png", new IAction2() {
                @Override
                public void perform(Object object) {
                    CRes.onSaveToFile((Image) object, "laser");
                }
            });
            Bullet.cannon2 = Bullet.filePack.loadImage("cannon.png", new IAction2() {
                @Override
                public void perform(Object object) {
                    CRes.onSaveToFile((Image) object, "cannon");
                }
            });
            Bullet.chatlong = Bullet.filePack.loadImage("chatlong.png", new IAction2() {
                @Override
                public void perform(Object object) {
                    CRes.onSaveToFile((Image) object, "chatlong");
                }
            });
            Bullet.filePack.loadImage("boomerang-big.png", new IAction2() {
                @Override
                public void perform(Object object) {
            ...
            Bullet.superShoot = mImage.createImage("/effect/no.png");
            Bullet.axit = mImage.createImage("/item/axit.png");
        } catch (Exception e) {
            e.printStackTrace();
        }
        Bullet.filePack = null;
        Bullet.BULLset_WIND_AFFECT = null;
        Bullet.BULLset_WEIGHT = null;
        Bullet.webId = 200;
    }
}
