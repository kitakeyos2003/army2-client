...
public class AvMain {
    ...
    public static void paintLoadImg(mGraphics g, int f, int xp, int yp, int anchor) {
        if (f >= 0) {
            AvMain.imgLoadImage.drawFrame(f, xp, yp, 0, anchor, g);
        } else {
            AvMain.imgLoadImage.drawFrame(CCanvas.gameTick / 2 % AvMain.imgLoadImage.nFrame, xp, yp, 0, anchor, g);
        }
    }

    public void update() {
        update_cmx_TabName(AvMain.cmx_tabnamemax);
    }
    ...
    public static void paintRect_Item(mGraphics g, int x, int y, int w, int h, int wItem, boolean isSelect,
            int typeCheck, int[] color) {
    }

    public static void paintRect_Item(mGraphics g, int x, int y, int w, int h, int wItem, boolean isSelect,
            int typeCheck) {
        int[] color = { -10928849, -8300224, -13359589 };
        if (isSelect) {
            color = new int[] { -3308998, -1400237, -9944034 };
        }
    }
    ...
    public static void paint_MainTab(mGraphics g, int xInTab, int yInTab, int wInTab, int hInTab, int typeLink,
            int numLink, int wItemBox, boolean isPaintDarkBg) {
        int h2 = mImage.getImageHeight(AvMain.imgTab[2].image);
        int w2 = mImage.getImageWidth(AvMain.imgTab[2].image);
        int h3 = mImage.getImageHeight(AvMain.imgTab[7].image);
        int w3 = mImage.getImageWidth(AvMain.imgTab[7].image);
        int w4 = mImage.getImageWidth(AvMain.imgTab[1].image);
        int h4 = mImage.getImageHeight(AvMain.imgTab[1].image);
        int h5 = mImage.getImageHeight(AvMain.imgTab[3].image);
        if (isPaintDarkBg) {
            g.setColor(-11646386);
        } else {
            g.setColor(-2982612);
    ...
    public static void update_cmx_TabName(int max) {
        if (AvMain.cmx_tabnamemax_old != AvMain.cmx_tabnamemax) {
            AvMain.cmx_tabnamemax_old = AvMain.cmx_tabnamemax;
            reset_cmx_tabname();
        }
        if (max > 0) {
            if (AvMain.cmx_tabname >= AvMain.cmx_tabnamemax) {
                ++AvMain.count_cmx_tabname;
                if (AvMain.count_cmx_tabname > 50) {
                    reset_cmx_tabname();
                }
            } else if (AvMain.time_cmx_tabname - CCanvas.timeNow < 0L) {
                ++AvMain.cmx_tabname;
            }
        }
    }
    ...
    public static void reset_cmx_tabname() {
        AvMain.cmx_tabname = 0;
        AvMain.count_cmx_tabname = 0;
        AvMain.time_cmx_tabname = CCanvas.timeNow + 1500L;
    }

    public static void paint_LinkTab(mGraphics g, int x, int y, int wcur, int type, int num) {
        g.setColor(-15463396);
        for (int i = 0; i < num; ++i) {
            int xp = (type == 0) ? (x + i * wcur) : x;
            int yp = (type == 0) ? y : (y + i * wcur);
            int trans = (type == 0) ? 0 : 4;
            mImage img = AvMain.imgTab[3];
            if (trans == 4) {
                img = AvMain.imgTab[17];
            }
        }
    }
    ...
    public static void paintbutton_vuong(mGraphics g, int Indexcolor, int xBut, int yBut, int wBut, int hBut) {
        FrameImage fra = (Indexcolor == 0) ? AvMain.frabtn_vuong0 : AvMain.frabtn_vuong1;
        FrameImage fra2 = (Indexcolor == 0) ? AvMain.frabtn_vuong_1_0 : AvMain.frabtn_vuong_1_1;
    ...
    public static void paintRectText(mGraphics g, int xText, int yText, int wText, int hText, boolean isFocus) {
        int wb = AvMain.fraTextField.frameWidth;
        int hb = AvMain.fraTextField.frameHeight;
        int wp = wText - 2 * wb;
        int hp = hText - 2 * hb;
        g.setColor(-8041424);
        int n = wp / wb;
    ...
    public void updatePointer() {
        boolean isTouch = CCanvas.isTouch;
    }
    ...
    public void paintWarning(mGraphics g, int x, int y, int w) {
        if (this.fraWarning != null) {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                int yp = y - this.fraWarning.frameHeight / 3;
                if (i == n - 1) {
    ...
    public static void paintIconString(mGraphics g, String str, mFont f, FrameImage fra, int nframe, int beginframe,
            int x, int y, int align, int w2w, mFont fBorder) {
        try {
            if (fra == null) {
                return;
            }
            ...
            int n = y + (fra.frameHeight - f.getHeight()) / 2 + 1;
        } catch (Exception ex) {
        }
    }
    ...
    public static void paintDeley(mGraphics g, int x, int y, int w, int h) {
        ...
        for (int nh = h / wd + ((h % wd > 0) ? 1 : 0), j = 0; j < nh; ++j) {
            for (int i = 0; i < nw; ++i) {
            }
        }
        g.restoreCanvas();
    ...
    static {
        AvMain.sizeUpgradeEff = new int[] { 2, 1, 1 };
        AvMain.colorUpgradeEffect = new int[][] { { -1, -1776411, -3289393, -5066061, -7105388, -8882056 },
                { -16715264, -16718592, -16724992, -16731392, -16738048, -16744448 },
                { -3276545, -4718363, -6094644, -7405389, -8781671, -10092416 },
                { -33024, -1740032, -3381760, -5023488, -6730752, -8372224 },
                { -16740097, -16743707, -16749108, -16752717, -16756071, -16759680 },
                { -1024, -1714176, -4142080, -5000960, -6713344, -8486912 },
                { 16711705, 15007767, 13369364, 11730962, 10027023, 8388621 } };
        AvMain.imghitScr = new mImage[3];
        AvMain.imgTab = new mImage[22];
        AvMain.color = new int[] { -15463396, -855308 };
    }
}
