...
public class Boss extends CPlayer {
    ...
    public Boss(int IDDB, byte index, int X, int Y, boolean isCom, int look, byte gunType, int maxHp) {
        super(IDDB, index, X, Y, isCom, look, gunType, null, maxHp);
        this.xWeb = new int[6];
        this.yWeb = new int[6];
    ...
    public void paintBigRobot(mGraphics g) {
        if (this.state == 5) {
            g.drawImage(Boss.robotInjured, this.x, this.y, 33, false);
            return;
        }
        if (this.curFrame > 3) {
            this.curFrame = 1;
        }
        g.drawImage(Boss.robotLeg, this.x, this.y, 33, false);
        ...
        g.drawImage(Boss.robotBody, this.x - 1, this.y - 10 + this.deltaY, 33, false);
        ...
        if (this.isBum) {
            ...
            g.drawRegion(Boss.fire, 0, 10 * this.fireFrame, 5, 10, 0, this.x - 10, this.y, 17, false);
            g.drawRegion(Boss.fire, 0, 10 * this.fireFrame, 5, 10, 0, this.x + 10, this.y, 17, false);
            g.drawRegion(Boss.fire, 0, 10 * this.fireFrame, 5, 10, 0, this.x - 15, this.y, 17, false);
            g.drawRegion(Boss.fire, 0, 10 * this.fireFrame, 5, 10, 0, this.x + 15, this.y, 17, false);
        }
    }

    public void paintKhiCau(mGraphics g) {
        g.drawImage(Boss.khicau, this.x, this.y + Boss.deltaYKC, 33, false);
        g.drawRegion(Boss.fan1, 0, this.fanFrame * 23, 4, 23, 0, this.x - 8, this.y - 18 + Boss.deltaYKC, 33, false);
        if (this.fanFrame == 0) {
            this.fanFrame = 1;
        ...
        if (CCanvas.gameTick % 2 == 0) {
            g.drawImage(Boss.fan2, this.x + 10, this.y - 35 + Boss.deltaYKC, 33, false);
            g.drawImage(Boss.fan2, this.x + 34, this.y - 35 + Boss.deltaYKC, 33, false);
        }
        g.drawImage(Boss.front_fan, this.x - 5, this.y - 43 + Boss.deltaYKC, 33, false);
        g.drawRegion(Boss.fan1, 0, this.fanFrame * 23, 4, 23, 0, this.x, this.y - 46 + Boss.deltaYKC, 33, false);
        int[] xinjured = {this.x, this.x - 30, this.x + 20};
        int[] yinjured = {this.y - 20, this.y - 35, this.y - 25};
        if (this.state == 5) {
            for (int i = 0; i < 3; ++i) {
                g.drawRegion(Boss.injured, 0, i * 12, 14, 12, 0, xinjured[i], yinjured[i] + Boss.deltaYKC, 0, false);
            }
        }
    }

    public void checkFrameBalloonGun() {
        if (Boss.xTo <= this.x + 50 && Boss.xTo >= this.x - 50) {
            this.curFrame = 1;
        } else if (Boss.xTo > this.x + 50) {
            this.curFrame = 0;
            this.look = 0;
        } else if (Boss.xTo < this.x - 50) {
            this.curFrame = 0;
    ...
    public void paintFan1(mGraphics g) {
        int d = (this.state == 5) ? 0 : Boss.deltaYKC;
        g.drawImage(Boss.back_fan, this.x, this.y + d, 33, false);
    }
    ...
    public void paintEye(mGraphics g) {
        if (this.state == 5) {
            this.pFrameImg.drawFrame(2, this.x + 2, this.y + Boss.deltaYKC - 1, 0, 33, g);
            return;
        }
        if (CCanvas.gameTick % 2 == 0) {
            this.pFrameImg.drawFrame(0, this.x + 2, this.y + Boss.deltaYKC - 1, 0, 33, g);
        } else {
            this.pFrameImg.drawFrame(1, this.x + 2, this.y + Boss.deltaYKC - 1, 0, 33, g);
        }
    }
    ...
    public void paintGunKhiCau(mGraphics g) {
        ...
        int d = (this.state == 5) ? 0 : Boss.deltaYKC;
        g.drawImage(Boss.gunkhicau, this.x, this.y - 7 + d, mGraphics.BOTTOM | mGraphics.HCENTER, false);
        this.pFrameImg.drawFrame(this.curFrame, this.x + 2, this.y + d - 7, this.look, 3, g);
    }

    public void paintBombKhiCau(mGraphics g) {
        int d = (this.state == 5) ? 0 : Boss.deltaYKC;
        g.drawImage(Boss.bombKhiCau, this.x, this.y + d, 33, false);
    }

    public void paintUFO(mGraphics g) {
        if (this.state == 8) {
            g.drawRegion(Boss.imgUFO, 0, 46, 51, 46, 0, this.x + this.dxUFO, this.y + this.dyUFO, 33, false);
        } else if (this.state == 5) {
            g.drawRegion(Boss.imgUFO, 0, 92, 51, 46, 0, this.x + this.dxUFO, this.y + this.dyUFO, 33, false);
        } else {
            g.drawRegion(Boss.imgUFO, 0, 0, 51, 46, 0, this.x + this.dxUFO, this.y + this.dyUFO, 33, false);
        }
        if (this.state == 5) {
            return;
        }
        if (CCanvas.gameTick % 3 == 0) {
            g.drawRegion(Boss.imgUFOFire, 0, 0, 16, 11, 0, this.x + this.dxUFO, this.y + this.dyUFO - 5, 17, false);
        } else {
            g.drawRegion(Boss.imgUFOFire, 0, 11, 16, 11, 0, this.x + this.dxUFO, this.y + this.dyUFO - 5, 17, false);
        }
    ...
    public void paintSpider(mGraphics g) {
        ...
        for (int i = 0; i < 3; ++i) {
            g.drawImage(Boss.web, this.xWeb[i], this.yWeb[i], 3, false);
        }
        if (this.sLook == 0) {
            g.drawRegion(Boss.spider, 0, this.fspider * 22, 41, 22, 1, this.x + 1, this.y, 33, false);
            g.drawRegion(Boss.spider, 0, this.fspider * 22, 41, 22, 0, this.x, this.y - 22, 33, false);
        }
        if (this.sLook == 1) {
            g.drawRegion(Boss.spider, 0, this.fspider * 22, 41, 22, 3, this.x, this.y, 33, false);
            g.drawRegion(Boss.spider, 0, this.fspider * 22, 41, 22, 2, this.x, this.y - 22, 33, false);
        }
        if (this.sLook == 2) {
            g.drawRegion(Boss.spider, 0, this.fspider * 22, 41, 22, 4, this.x, this.y, 40, false);
            g.drawRegion(Boss.spider, 0, this.fspider * 22, 41, 22, 5, this.x + 22, this.y, 40, false);
        }
        if (this.sLook == 3) {
            g.drawRegion(Boss.spider, 0, this.fspider * 22, 41, 22, 6, this.x, this.y, 40, false);
            g.drawRegion(Boss.spider, 0, this.fspider * 22, 41, 22, 7, this.x + 22, this.y, 40, false);
        }
        if (this.isCapture) {
            g.setColor(5395026);
    ...
    public void paintBugRobot(mGraphics g) {
        ...
        int xBody = this.x - this.deltaX2 + ((this.curFrame == 1 || this.curFrame == 2) ? this.xBugBack : 0);
        int yBody = this.y - 10 + this.deltaY + ((this.curFrame == 2 || this.curFrame == 3) ? this.yBugBack : 0);
        int xGun = this.x + this.deltaX1 + ((this.curFrame == 1 || this.curFrame == 2) ? this.xBugBack : 0);
        int yGun = this.y - 15 + this.deltaY + this.deltaY1 + ((this.curFrame == 2 || this.curFrame == 3) ? this.yBugBack : 0);
        g.drawRegion(Boss.bugbody, 0, 30 * this.framebd_1, 42, 30, LOOK, xBody, yBody, mGraphics.BOTTOM | mGraphics.HCENTER, false);
        g.drawRegion(Boss.bugleg, 0, 25 * this.frameleg_1, 44, 25, LOOK, this.x, this.y, mGraphics.BOTTOM | mGraphics.HCENTER, false);
    ...
    public void paintGhost(mGraphics g, int type) {
        g.drawRegion((type == 0) ? Boss.ghost : Boss.ghost2, 0, this.gFrame * 32, 35, 32, this.look, this.x, this.y, 33, false);
    }
    ...
    public void paint(mGraphics g) {
        ...
        if (this.gun == 14 && (this.isFly || this.isBum)) {
            ...
            g.drawRegion(Boss.fire, 0, 10 * this.fireFrame, 5, 10, 0, this.x - 5, this.y + 2, mGraphics.HCENTER | mGraphics.TOP, false);
            g.drawRegion(Boss.fire, 0, 10 * this.fireFrame, 5, 10, 0, this.x + 5, this.y + 2, mGraphics.HCENTER | mGraphics.TOP, false);
        }
        if (this.gun == 25) {
            this.paintGhost(g, 0);
        ...
        if (this.gun == 23) {
            if (this.state != 5 && this.hp > 0) {
                g.drawImage(Boss.gift_1, this.x, this.yGift_1, 33, false);
            }
            return;
        }
        if (this.gun == 24) {
            if (this.state != 5) {
                ...
                g.drawImage(Boss.gift_2, this.x, this.yGift_2 - 8, 33, false);
                g.drawRegion(Boss.bongbong, 0, this.frameG * 35, 35, 35, 0, this.x, this.yGift_2, 33, false);
            }
            return;
        }
        super.paint(g);
            ...
            case 17: {
                ...
                if (this.flyActive) {
                    ...
                    if (this.bulletType == 43) {
                        Boss.camY += 2;
                        GameScr.cam.setTargetPointMode(this.x, this.y + Boss.camY);
                    }
                } else {
                    Boss.camY = 0;
                    Boss.deltaYKC = PM.deltaYKC;
                }
    ...
    static {
        try {
            Boss.gift_1 = mImage.createImage("/item/box.png");
            Boss.bongbong = mImage.createImage("/item/bongbong.png");
            Boss.gift_2 = mImage.createImage("/item/box2.png");
            Boss.gift_empty = mImage.createImage("/item/boxEmpty.png");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
